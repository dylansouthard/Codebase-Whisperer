...
def test_ensure_chunks_and_vec_cache_tables_exist(tmp_path: Path):
    ...
    for name in ["id","path","realpath","is_symlink","relpath","lang","symbol",
    ...
    # smoke: vector column is list<float32>[dim]
...
def test_try_add_missing_columns_adds_new_fields(tmp_path: Path):
    db = open_db(str(tmp_path / "ldb"))
    # Create a minimal table missing a couple of columns
    minimal_schema = pa.schema([
        ("id", pa.string()),
        ("vector", pa.list_(pa.float32(), 8)),
    ])
    tbl = ensure_table(db, "minichunks", minimal_schema)
...
def test_upsert_rows_inserts_and_merges(tmp_path: Path):
    db = open_db(str(tmp_path / "ldb"))
    tbl = ensure_table(db, "rows", pa.schema([
        ("id", pa.string()),
        ("value", pa.int32()),
    ]))
